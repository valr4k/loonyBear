...
struct CoreDataHabitRepositoryTests {
    ...
    func restoreHabitWritesArchivedGapWithoutOverwritingExplicitHistory() throws {
        ...
        var editDraft = EditHabitDraft(
            ...
            completedDays: restoredDetails.completedDays,
        ...
        #expect(updatedDetails.name == "Restored habit updated")
    }

    @Test
    func restoreHabitAsNewClearsHistoryAndUsesActiveFromAsStartDate() throws {
        let calendar = Calendar(identifier: .gregorian)
        var now = TestSupport.makeDate(2026, 5, 3, calendar: calendar).addingTimeInterval(10 * 60 * 60)
        let persistence = PersistenceController(inMemory: true)
        ...
        var draft = CreateHabitDraft()
        draft.type = .build
        draft.name = "Restore as new habit"
        draft.startDate = TestSupport.makeDate(2026, 5, 1, calendar: calendar)
        draft.scheduleRule = .weekly(.daily)

        let habitID = try repository.createHabit(from: draft)
        try repository.completeHabitDay(id: habitID, on: TestSupport.makeDate(2026, 5, 2, calendar: calendar))
        try repository.skipHabitDay(id: habitID, on: TestSupport.makeDate(2026, 5, 3, calendar: calendar))
        try repository.setHabitArchived(id: habitID, isArchived: true)
        ...
        #expect(restoredDetails.name == "Fresh habit")
        ...
        #expect(restoredDetails.currentStreak == 0)
        #expect(restoredDetails.longestStreak == 0)
        #expect(restoredDetails.totalCompletedDays == 0)
        #expect(restoredDetails.completedDays.isEmpty)
        ...
        #expect(try TestSupport.habitHistoryBucketCount(habitID: habitID, context: context) == 0)
        #expect(try TestSupport.habitHistoryRangeCount(habitID: habitID, context: context) == 0)
    }

    @Test
    func restoreHabitClearsStaleFutureArchivedDaysWhenActiveFromMovesEarlier() throws {
        let calendar = Calendar(identifier: .gregorian)
        ...
        let firstArchivedDetails = try #require(try repository.fetchHabitDetails(id: habitID))
        var firstRestoreDraft = EditHabitDraft(
            id: habitID,
            type: firstArchivedDetails.type,
            ...
            completedDays: firstArchivedDetails.completedDays,
        ...
        try repository.setHabitArchived(id: habitID, isArchived: true)
        let secondArchivedDetails = try #require(try repository.fetchHabitDetails(id: habitID))
        var secondRestoreDraft = EditHabitDraft(
            id: habitID,
            type: secondArchivedDetails.type,
            ...
            completedDays: secondArchivedDetails.completedDays,
    ...
    @Test
    func updateHabitUsesSelectedScheduleEffectiveFrom() throws {
        let calendar = Calendar(identifier: .gregorian)
        let now = TestSupport.makeDate(2026, 5, 3, calendar: calendar)
        let persistence = PersistenceController(inMemory: true)
        let context = persistence.container.viewContext
        ...
        #expect(latest.value(forKey: "effectiveFrom") as? Date == TestSupport.makeDate(2026, 5, 5, calendar: calendar))
        #expect(CoreDataScheduleSupport.rule(from: latest) == .weekly(.monday))
    }

    @Test
    func updateHabitAutomaticallyUsesTodayAsEffectiveFrom() throws {
        let calendar = Calendar(identifier: .gregorian)
    ...
    @Test
    func updateFutureHabitAutomaticallyUsesStartDateAsEffectiveFrom() throws {
        let calendar = Calendar(identifier: .gregorian)
        let now = TestSupport.makeDate(2026, 5, 4, calendar: calendar)
        let persistence = PersistenceController(inMemory: true)
        let context = persistence.container.viewContext
    ...
    @Test
    func updateHabitRemovesFutureScheduleVersionsReplacedByEarlierEffectiveFrom() throws {
        let calendar = Calendar(identifier: .gregorian)
        let now = TestSupport.makeDate(2026, 5, 4, calendar: calendar)
        let persistence = PersistenceController(inMemory: true)
        let repository = CoreDataHabitRepository(
