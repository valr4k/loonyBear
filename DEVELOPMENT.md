...
- Auto Backup cannot remain enabled without a trusted backup destination. A trusted destination means the selected folder is usable and the current backup state is one of: no backup file yet, a backup last created by this app install, or a backup last restored by this app install. A folder containing a readable but not-yet-restored backup, an unreadable/corrupt backup, or an unavailable bookmark must keep Auto Backup Off. Enabling without a folder prompts folder selection; enabling with an unavailable remembered folder prompts reselection; enabling with an untrusted readable/unreadable backup keeps the toggle Off and shows an informational banner. Startup, foreground, background, and debounced backup checks must disable Auto Backup if the destination stops being trusted.
...
- Auto Backup destination trust behavior: enabling without a folder, enabling with an unavailable remembered folder, picker cancellation, empty/created/restored folder selection enabling the toggle, readable-but-unrestored backup selection keeping it Off, unreadable backup selection keeping it Off, startup/foreground/background disabling when trust is lost, and no immediate Auto Backup after successful manual restore
