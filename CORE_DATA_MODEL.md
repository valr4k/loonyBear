...
- Restore has two persistence modes. `Continue Progress` preserves existing bucket, range, and legacy history rows and writes archived gap rows only into empty days. `Start From Scratch` deletes all bucket, range, and legacy history rows for the item, sets `startDate` to `activeFrom`, clears `isArchived`/`archivedAt`, and inserts a new initial schedule version effective from the new Start Date.
- Repository restore is result-bearing and idempotent at the persistence boundary. `restoreHabit` and `restorePill` return `true` only when the stored row existed and was archived at the time of the write. They return `false` when the row is missing or already active, and in that case no stored fields, schedule versions, history buckets/ranges, legacy rows, overdue anchors, or archive facts are changed.
- AppState treats the repository restore result as the source of truth for side effects. Restore-specific notification refresh, delivered notification cleanup, snooze cleanup, badge refresh, and widget/archive side effects run only after a `true` restore result. This prevents stale UI, repeated taps, or already-restored rows from triggering notification reschedules for a restore that did not happen.
...
Auto Backup does not add Core Data entities and does not change the backup archive schema. Its enabled flag, dirty flag, and dirty generation are local `UserDefaults` values, so restoring a backup does not overwrite the user's Auto Backup preference. Auto Backup still requires a trusted selected destination: a usable folder with no backup yet, a backup last created by this app install, or a backup last restored by this app install. If the folder is missing, inaccessible, unreadable, or contains a readable backup that has not been restored by this app install, the local enabled flag is turned off until the user explicitly creates/restores a trusted backup baseline.
