...
- Auto Backup is user-controlled from the Backup screen only, but it cannot stay enabled unless the selected backup destination is trusted. A trusted destination is a usable folder with no backup yet, a backup last created by this app install, or a backup last restored by this app install. A readable backup that has not been restored, an unreadable backup, or an unavailable folder keeps Auto Backup Off. It reuses the normal backup writer, file names, schema, rotation, and folder bookmark. Dirty app data is marked after write operations, notification action mutations, Event changes, Pill reorder, and backed-up Settings changes, then automatic backup runs after a 20-second debounce when the destination is trusted. If destination trust is lost, Auto Backup disables itself.
