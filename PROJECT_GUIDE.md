...
- Auto Backup is user-controlled from the Backup screen only, but it cannot stay enabled without a usable selected folder. It reuses the normal backup writer, file names, schema, rotation, and folder bookmark. Dirty app data is marked after write operations, notification action mutations, Event changes, Pill reorder, and backed-up Settings changes, then automatic backup runs after a 20-second debounce when the selected folder is usable. If the folder becomes unavailable, Auto Backup disables itself and waits for the user to choose a usable folder again.
