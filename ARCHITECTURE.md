...
- Archived Habit and Pill Details are read-only by default. Restore dismisses the read-only sheet and opens a separate Restore Draft sheet: the item stays archived until the Restore Draft sheet is saved, `Active From` is chosen in a bounded window, and `End Repeat` defaults to Never inside the draft. Saving a valid Restore Draft shows a system choice between `Keep History` and `Start Fresh`. `Keep History` starts a new schedule version at Active From, removes stale archived history on/after Active From, writes archived gap rows only into empty days from `archivedAt` through the day before Active From, and auto-fills scheduled past days from Active From through yesterday as restored completed/taken when empty. `Start Fresh` clears all old history rows, sets Start Date to Active From, starts a new schedule version at Active From, and returns the item with empty calendar history and reset streak/count totals. If the user changes tabs before the Restore Draft handoff opens, the pending handoff is cancelled.
